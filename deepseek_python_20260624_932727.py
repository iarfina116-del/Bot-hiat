...
# --- Configuration ---
TOKEN = '8353107925:AAFUfj7rvnBtCA6jUcnKwH8f8Pkjx55Mu40' # Replace with your actual token
OWNER_ID = 8855368882 # Replace with your Owner ID
ADMIN_ID = 8855368882 # Replace with your Admin ID (can be same as Owner)
YOUR_USERNAME = '@NON_FUCKER_19x' # Replace with your Telegram username (without the @)
UPDATE_CHANNEL = 'https://t.me/HYDRA_PAPA' # Replace with your update channel link
...
# --- NEW: Helper Functions for Script Limit ---
def get_user_active_script_count(user_id):
    ...
    for script_key, script_info in list(bot_scripts.items()):
        try:
            # script_key format: "{user_id}_{file_name}"
...
def can_user_start_new_script(user_id):
    """Check if user can start a new script (max 1 running script per user)"""
    # Admin and Owner have no limit
    if user_id in admin_ids or user_id == OWNER_ID:
        return True
    running_count = get_user_active_script_count(user_id)
    return running_count < 1  # Maximum 1 script per user
# --- END: Helper Functions for Script Limit ---
...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    # --- NEW: Check if user already has a running script ---
    if not can_user_start_new_script(script_owner_id):
        running_count = get_user_active_script_count(script_owner_id)
        bot.reply_to(message_obj_for_reply, 
                     f"❌ আপনি ইতিমধ্যে {running_count} টি স্ক্রিপ্ট চালাচ্ছেন! দয়া করে আগের স্ক্রিপ্ট থামান (`/checkfiles`) তারপর নতুন চালান।\n"
                     f"⚠️ প্রতিটি ইউজার সর্বোচ্চ ১টি স্ক্রিপ্ট চালাতে পারেন।")
        return
    # --- END: New Check ---
...
def start_bot_callback(call):
    try:
        ...
        # --- NEW: Check if user already has a running script (except admin/owner) ---
        if requesting_user_id not in admin_ids and requesting_user_id != OWNER_ID:
            if not can_user_start_new_script(requesting_user_id):
                running_count = get_user_active_script_count(requesting_user_id)
                bot.answer_callback_query(call.id, 
                    f"❌ আপনি ইতিমধ্যে {running_count} টি স্ক্রিপ্ট চালাচ্ছেন! দয়া করে আগের স্ক্রিপ্ট থামান।\n⚠️ প্রতিটি ইউজার সর্বোচ্চ ১টি স্ক্রিপ্ট চালাতে পারেন।", 
                    show_alert=True)
                return
        # --- END: New Check ---
